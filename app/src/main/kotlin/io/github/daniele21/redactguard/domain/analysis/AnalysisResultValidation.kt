...
internal object AnalysisResultParser {
    ...
    fun parse(raw: String): List<UnvalidatedFinding> {
        ...
        val schemaVersion = (root.fields["schemaVersion"] as? JsonValue.IntegerValue)?.value
        ...
        val findings = (root.fields["findings"] as? JsonValue.ArrayValue)?.values
            ?: fail(AnalysisResultFailureCode.UNEXPECTED_ROOT_SHAPE)
    ...
    private fun parseFinding(value: JsonValue): UnvalidatedFinding {
        ...
        val typeId = (objectValue.fields["typeId"] as? JsonValue.StringValue)?.value
            ?: fail(AnalysisResultFailureCode.UNEXPECTED_FINDING_SHAPE)
        val surface = (objectValue.fields["surface"] as? JsonValue.StringValue)?.value
            ?: fail(AnalysisResultFailureCode.UNEXPECTED_FINDING_SHAPE)
        val segmentId = (objectValue.fields["segmentId"] as? JsonValue.StringValue)?.value
            ?: fail(AnalysisResultFailureCode.UNEXPECTED_FINDING_SHAPE)
        if (
...
internal object FindingValidator {
    fun validate(
        ...
        val sourceIndex = buildSourceIndex(chunks, canonicalSegments)
        ...
        val deduplicated = validated.distinctBy { Triple(it.typeId, it.source.segmentId, it.source.range) }
            .sortedWith(compareBy({ it.source.segmentId.value }, { it.source.range.startInclusive }, { it.source.range.endExclusive }, { it.typeId.value }))
    ...
    private fun buildSourceIndex(
        ...
        fragmentsByBase.forEach { (baseId, fragments) ->
            ...
            fragments.forEachIndexed { index, fragment ->
                ...
                if (fragment.segmentId != expectedId || result.put(fragment.segmentId, SourceMapping(canonical.id, offset)) != null) return null
    ...
    private fun exactOccurrences(text: String, surface: String): List<Int> {
