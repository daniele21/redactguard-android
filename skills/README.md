...
- `validate-change` — choose the narrowest sufficient iterative validation, select truthful E2E environment fidelity and route unavailable deterministic gates to remote automation rather than human execution;
- `preflight-change` — establish exact-head readiness after base/diff review, `LEAN|SCOPED|STRONG|FULL` selection, E2E journey/fidelity selection and execution-capability classification;
...
Before publication, `preflight-change` owns profile/executor/fidelity/readiness selection. When it reports `READY_FOR_REMOTE_PREFLIGHT`, use `remote-preflight` immediately. Automatable Gradle/R8/Lint/build/emulator work must not fall back to the user merely because the current agent lacks Android tooling. Physical-device evidence remains separate and may satisfy only the residual gaps declared in `.engineering/e2e.json`.
