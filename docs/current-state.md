...
Completed implementation slices: VUI-8, VUI-9, VUI-10, VUI-15, VUI-11, VUI-12, VUI-13 and VUI-14. VUI-16/17 implementations are present in the final candidate and are READY for their exact-head workflow evidence. VUI-18 owns freshness, complete diff review, final automated evidence and integration to `dev`.

Execution remaining:
1. move the clean, fresh fidelity candidate onto PR #132;
2. run FULL remote preflight + Visual Evidence v2 + Emulator E2E v2 on one exact head;
3. explicitly review the target-vs-actual visual artifact;
4. merge VUI-18 to `dev` if all automated evidence agrees;
