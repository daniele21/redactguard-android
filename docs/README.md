...
| Question | Source |
| --- | --- |
| What durable product mission/users/problems/value/boundaries/principles govern decisions? | `product.md` |
| What is RedactGuard and why does it exist publicly? | README identity sections: title/summary/why, audience and primary outcome |
...
| How does product-change shaping route? | `../.engineering/product.json` + `../skills/shape-product-change/SKILL.md` |
...
- **Identity** — public purpose, primary audience/outcome and stable positioning. Do not rewrite this merely because implementation, commands, configuration or a feature workflow changed.
...
`product.md` is different from README identity: it is the concise decision source for durable users/problems/value/differentiation/non-goals/principles/quality promises. Update it only when those durable product truths change, not for every feature implementation.

A change may legitimately report `PRODUCT_SOURCE: N/A`, `README_IDENTITY: N/A` and `README_USAGE: UPDATED`.
...
- `workstreams/harness-control-plane-consumer-cutover.md` — multi-preset tolerance is integrated; assigned-use-case discovery and activation lifecycle depend on the corresponding Harnex SDK/control-plane capabilities.

Keep ownership narrow: Harnex owns application/use-case/preset/model/residency/Host telemetry administration; RedactGuard owns only the consumer-safe selection/lifecycle state exposed by the published SDK plus document/PII/review/export product behavior.

## Rules

- one canonical owner per durable fact;
- product strategy truth belongs in `product.md`; temporary discovery reasoning stays in the issue/workstream/task;
