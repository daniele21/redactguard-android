...
Possible evidence includes existing usage/support data, representative-user observation/feedback, prototype/usability evidence, dogfooding, benchmark or technical spike, compatibility experiment, alternative/market evidence, privacy-safe telemetry or a bounded rollout experiment.

A discovery step may validly conclude `BUILD`, `NARROW_SCOPE`, `CHOOSE_ALTERNATIVE` or `DO_NOT_BUILD`.
...
Product-impact evidence need not be analytics. It can be usability evidence, support friction, task success, operational quality, adoption or another proportionate signal. Never report `shipped` as equivalent to confirmed product impact.
...
For feature/strategic changes ask what useful product question can only real use answer after release. If none, mark `N/A`. If material, record a bounded question and the least invasive useful evidence. Do not add telemetry merely because the product workflow mentions observation.

## Output discipline

For a material change, produce no more than needed:
...
Surface the conflict instead of improvising when the requested solution contradicts durable product mission/non-goals; a material high-risk assumption has no evidence and implementation would be expensive/hard to reverse; quality requirements conflict with the solution; usability intent is unresolved while UI structure is being locked; feasibility evidence contradicts the product claim; or compatibility/deprecation would silently break supported consumers.
