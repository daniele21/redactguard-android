...
internal enum class JsonFailureCode {
    INPUT_TOO_LARGE, INVALID_JSON, DEPTH_EXCEEDED, CONTAINER_TOO_LARGE, STRING_TOO_LARGE, DUPLICATE_KEY,
}

internal class StrictJsonException(val code: JsonFailureCode) : IllegalArgumentException("Invalid RedactGuard JSON: $code")
...
internal class StrictJsonReader(
    ...
    private fun parseValue(cursor: JsonCursor, depth: Int): JsonValue {
    ...
    private fun parseObject(cursor: JsonCursor, depth: Int): JsonValue.ObjectValue {
        cursor.expect('{')
        cursor.skipWhitespace()
    ...
    private fun parseArray(cursor: JsonCursor, depth: Int): JsonValue.ArrayValue {
        cursor.expect('[')
        cursor.skipWhitespace()
    ...
    private fun appendEscaped(cursor: JsonCursor, value: StringBuilder) {
    ...
    private fun appendUnicodeEscape(cursor: JsonCursor, value: StringBuilder) {
...
private class JsonCursor(private val input: String) {
    var position: Int = 0
        private set
    ...
    fun consumeIf(expected: Char): Boolean = if (peek() == expected) { position += 1; true } else false
    fun expect(expected: Char) { if (!consumeIf(expected)) fail() }
    fun skipWhitespace() { while (peek() in listOf(' ', '\n', '\r', '\t')) position += 1 }
    fun consumeDigits() { while (peek()?.isDigit() == true) position += 1 }
    fun consumeLiteral(literal: String, value: JsonValue): JsonValue { literal.forEach(::expect); return value }
    fun substring(start: Int, end: Int) = input.substring(start, end)
}
