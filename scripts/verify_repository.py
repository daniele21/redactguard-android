...
REQUIRED=("README.md","AGENTS.md","CONTRIBUTING.md","SECURITY.md","EXECUTION-CAPABILITY-CONTRACT.md",".engineering/baseline.json",".engineering/documentation-policy.json",".engineering/commands.json",".engineering/e2e.json",".github/pull_request_template.md",".github/workflows/repository-health.yml","docs/README.md","docs/architecture.md","docs/current-state.md","docs/features/README.md","docs/adr/README.md","docs/workstreams/README.md","scripts/verify_operations.py","scripts/verify_e2e.py","scripts/verify_stage_environment_policy.py","scripts/verify_product_experience.py","scripts/detect_ci_scope.py")
PLACEHOLDERS=("<PROJECT_NAME>","<REPLACE_WITH_","<DESCRIBE_","<LIST_")

def main()->int:
    parser=argparse.ArgumentParser(); parser.add_argument("--root",default="."); parser.add_argument("--template-mode",action="store_true"); args=parser.parse_args(); root=Path(args.root).resolve(); errors=[]; warnings=[]
    for rel in REQUIRED:
    ...
    try: baseline=json.loads((root/".engineering"/"baseline.json").read_text(encoding="utf-8"))
